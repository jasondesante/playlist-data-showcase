...
- [ ] Update `BeatMapSummary.tsx` to show subdivision info
  - [ ] Display subdivision types used (e.g., "quarter → eighth → half")
  - [ ] Show when SubdividedBeatMap exists
...
- [ ] Use `useSubdivisionPlayback` hook in BeatPracticeView
  - [ ] Initialize when entering practice mode
  - [ ] Pass UnifiedBeatMap as foundation
  - [ ] Configure with `immediate` transition mode (default)

### 6.5 Update BeatPracticeView

- [ ] Add subdivision UI to BeatPracticeView
  - [ ] Add SubdivisionButtons below controls
  - [ ] Show current subdivision in stats area
  - [ ] Handle subdivision button clicks via hook
