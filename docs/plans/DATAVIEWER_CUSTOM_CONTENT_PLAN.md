...
- [x] Custom class with audio_preferences works with ClassSuggester (verified 2026-02-26 - integration test created at src/__tests__/classSuggesterIntegration.test.ts, 11 tests pass confirming: ExtensionManager registration of custom classes with audio_preferences, ClassSuggester.suggest() returns custom classes with matching audio preferences, custom classes compete with default classes, getClassData() returns audio_preferences from custom classes. Note: Custom classes require weights to be set via manager.setWeights() for inclusion in ClassSuggester selection pool)
- [x] Custom race with subraces generates characters correctly (verified 2026-02-26 - integration test created at src/__tests__/customRaceSubracesIntegration.test.ts, 15 tests pass confirming: ExtensionManager registration of custom races with subraces, CharacterGenerator.generate() creates characters with custom race and subrace, subrace-specific traits are applied correctly via FeatureQuery.getRacialTraitsForSubrace(), base traits and subrace-specific traits are distinguished, prerequisite validation works for subrace-specific traits, custom races coexist with default races)
