...
**Combat AI Types:** `AIPlayStyle`, `AIConfig`, `AIDecision`, `AIThreatAssessment`, `CombatantMetrics` — see [CombatAI](#combatai)
...
> See [InnateSpell Interface](#innatespell-interface) under [SpellcastingGenerator](#spellcastinggenerator) for the full definition.
...
| `useTripleMeter` | `false` | Enable TPS3 triple meter resolution for 3/4 and 6/8 time signatures. Uses Ellis 2007 triple meter calculation (TPS3) to boost tempos with strong third-period evidence. Enable for waltzes, 6/8 shuffles, and other triple-meter music where beats occur in groups of three. Works independently of `useOctaveResolution` and can be enabled simultaneously. |
...
| `EASY_ACCURACY_THRESHOLDS` | Easy difficulty thresholds |
| `MEDIUM_ACCURACY_THRESHOLDS` | Medium difficulty thresholds |
| `HARD_ACCURACY_THRESHOLDS` | Hard difficulty thresholds |
