...
export const useHeroEquipment = (): UseHeroEquipmentReturn => {
    ...
    // Persist instance ID + reconciliation updates after render to avoid setState-during-render
    const pendingInstanceUpdateRef = useRef<CharacterSheet | null>(null);

    /**
     * Ensure all equipment items have instance IDs and reconcile equipped state.
     *
     * Reconciliation fixes a desync where equipment_effects has an item but the
     * inventory equipped flag is false (caused by old engine code where
     * result.applied was result.count > 0, which failed for items with no properties).
     */
    const activeCharacter = useMemo(() => {
        if (!rawActiveCharacter) return undefined;

        const equipment = rawActiveCharacter.equipment;
        if (!equipment) return rawActiveCharacter;

        let needsUpdate = false;

        // Build a set of equipment names that have active effects (source of truth)
        const equippedEffectNames = new Set<string>();
        if (rawActiveCharacter.equipment_effects) {
            for (const effect of rawActiveCharacter.equipment_effects) {
                equippedEffectNames.add(effect.source);
            }
        }
        ...
        const ensureIds = (items: EnhancedInventoryItem[]): EnhancedInventoryItem[] => {
            return items.map(item => {
                ...
                // Fix desync: if equipment_effects says it's equipped but flag says false, fix the flag
                if (!item.equipped && equippedEffectNames.has(item.name)) {
                    needsUpdate = true;
                    updated = { ...item, equipped: true };
                }

                // Add instance IDs to items that lack them
                if (!updated.instanceId) {
                    needsUpdate = true;
                    updated = {
                        ...updated,
                        instanceId: `${updated.name.toLowerCase().replace(/\s+/g, '_')}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
                    };
                }
        ...
        if (needsUpdate) {
            pendingInstanceUpdateRef.current = updatedCharacter;
        }
    ...
    useEffect(() => {
        const pending = pendingInstanceUpdateRef.current;
        if (pending) {
            pendingInstanceUpdateRef.current = null;
            logger.debug('HeroEquipment', 'Added instance IDs to equipment items', {
                characterName: pending.name
            });
            updateCharacter(pending);
        }
    });
    ...
    const equipItem = useCallback(async (itemId: string): Promise<EquipmentOperationResult> => {
        ...
        try {
            ...
            // Check if equip was blocked
            if (!result.applied) {
                const reason = result.errors.length > 0
                    ? result.errors.join('; ')
                    : 'Equipment failed to apply (no effects counted)';
                console.error('[equipItem] Failed to equip', equipmentData.name, { errors: result.errors, count: result.count });
    ...
    const addItemToInventory = useCallback(async (
        ...
        try {
            ...
            if (autoEquip) {
                ...
                if (!result.applied) {
                    logger.warn('HeroEquipment', 'Auto-equip blocked', { errors: result.errors });
                }
                updatedCharacter.equipment.equippedWeight = (updatedCharacter.equipment.equippedWeight || 0) + itemWeight;
            }
